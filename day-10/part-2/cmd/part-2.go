...
func main() {
	...
	blockedFor := 0
	currentCycle := 1
	cyclesSinceLastNewLine := 0
	valueToAddWhenUnblocked := 0
	firstCycle := true
	currentPixelBeingDrawn := 0
	...
	for i := 0; i <= lenCommandsStart; {
		...
		if x-1 == currentPixelBeingDrawn || x == currentPixelBeingDrawn || x+1 == currentPixelBeingDrawn {
			print("#")
		} else {
			print(".")
		}

		cyclesSinceLastNewLine++
		currentCycle++
		currentPixelBeingDrawn++

		if cyclesSinceLastNewLine == 40 {
			currentPixelBeingDrawn = 0
			cyclesSinceLastNewLine = 0
			println()
		}
