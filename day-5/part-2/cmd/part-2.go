...
func main() {
	...
	println("ArrayOfColumns generated")
	// Parse Instructions
	...
	for i := lineWithBoxCount + 2; i < len(dataArray); i++ {
		...
		arrayOfColumns = moveBox(arrayOfColumns, from, to, howManyToMove)
	}
...
func moveBox(array [][]string, from, to, howManyToMove int) [][]string {
	array[to-1] = append(array[to-1], array[from-1][len(array[from-1])-howManyToMove:len(array[from-1])]...)
	array[from-1] = remove(array[from-1], howManyToMove)
	return array
}

// Helper method to remove last entry from slice
func remove(slice []string, howManyToRemove int) []string {
	return slice[:len(slice)-howManyToRemove]
}
